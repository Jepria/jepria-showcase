...
/* group: Внутренние функции */

/* ifunc: createFeatureProcessInternal
  Служебная функция создания записи обработки запроса

  Параметры:
  featureId	                  - запрос на функционал
  featureStatusCode	          - статус запроса на функционал
  operatorId                  - пользователь, создающий запись

  Возврат:
  Идентификатор созданной записи обработки запроса на функционал
*/
function createFeatureProcessInternal(
  featureId	                  integer
...
begin
  ...
  logger.trace( 'Запрос ' || featureId || ' изменил статус на ' || featureStatusCode);
  return
    featureProcessId
  ;
exception when others then
  raise_application_error(
    ...
    , logger.errorStack(
        'Ошибка внутренней функции создания записи обработки запроса. ('
        || 'featureId="' || featureId || '"'
...
end createFeatureProcessInternal;
...
/* iproc: normalizeFeatureWorkSequence
  Устранение разреженности последовательности

*/
procedure normalizeFeatureWorkSequence
is
-- normalizeFeatureWorkSequence
begin
  ...
  using
    ...
    from
      ...
      select
        f.feature_id
        , f.work_sequence
        , row_number() over( order by f.work_sequence) as x_work_sequence
      from
        jrs_feature f
      where
        f.work_sequence > 0
    ...
    where
      t.x_work_sequence != t.work_sequence
    ) src
  on
    ( dst.feature_id = src.feature_id)
  when matched then update
    set
      dst.work_sequence = src.x_work_sequence
  ;
exception
  when others then
    raise_application_error(
      pkg_Error.ErrorStackInfo
      , logger.errorStack( 'Ошибка при удалинии пропусков в последовательности.')
      , true
    );
end normalizeFeatureWorkSequence;

/* proc: setFeatureWorkSequence
...
is
  ...
  -- Используемый ID оператора
  usedOperatorId      integer := coalesce( operatorId, pkg_Operator.getCurrentUserId());

  -- Максимальное значение последовательности
  maxWorkSequence     integer;

  -- Используемое значение последовательности
  usedWorkSequence    integer;

  -- Предыдущее значение последовательности
  oldWorkSequence     integer;
  ...
  */
  procedure updateWorkSequence
  is
  -- updateWorkSequence
  begin

    if usedWorkSequence is null then

      update
        jrs_feature f
      set
        f.work_sequence = null
      where
        ...
        select
          ...
          , case f.feature_id
              when featureId then
                ...
                case
                  when f.work_sequence < usedWorkSequence then
                    f.work_sequence
                  else
                    f.work_sequence + 1
                end
            end as work_sequence
        from
          jrs_feature f
        where
          f.work_sequence > 0
          or f.feature_id = featureId
        minus
        select
          f.feature_id
          , f.work_sequence
      ...
      when matched then update
        set
          dst.work_sequence = src.work_sequence
      ;
    end if;
  exception
    when others then
      raise_application_error(
        pkg_Error.ErrorStackInfo
        , logger.errorStack( 'Ошибка при обновлнеии последовательности.')
        , true
      );
  end updateWorkSequence;

-- setFeatureWorkSequence
begin

  -- Проверка наличия роли у пользователя
  pkg_Operator.isRole( usedOperatorId, AssignWrkSequenceFea_RoleSName);
  ...
  -- Получаем максимальное значение последовательности
  select
  ...
  -- Получаем номер фичи в последовательности
  if workSequence < 1 then
    usedWorkSequence := 1;
  elsif workSequence > maxWorkSequence + 1 then
    usedWorkSequence := maxWorkSequence + 1;
  else
    usedWorkSequence := workSequence;
  end if;

  -- Обновляем последовательность
  updateWorkSequence();

  -- Избавляемся от разреженности последовательности
  normalizeFeatureWorkSequence();

  -- Установка статуса "Установлен порядок выполнения"
  if oldWorkSequence is null then
    featureProcessId :=
      createFeatureProcessInternal(
        featureId         => featureId
      , featureStatusCode	=> Sequenced_FeatureStatusCode
      , operatorId	      => operatorId
      );
  end if;
exception
  when others then
    raise_application_error(
      ...
      , logger.errorStack(
          ...
          || ' featureId=' || to_char( featureId)
          || ', workSequence=' || to_char( workSequence)
          || ', operatorId=' || to_char( operatorId)
          || ')'
        )
      , true
    );
end setFeatureWorkSequence;
...
begin
  ...
  featureProcessId :=
    createFeatureProcessInternal(
      featureId         => featureId
    , featureStatusCode	=> New_FeatureStatusCode
    , operatorId	      => operatorId
    );
  return featureId;
exception when others then
...
begin
  ...
  if workSequence is not null then
    -- Избавляемся от разреженности последовательности
    normalizeFeatureWorkSequence();
...
begin
  ...
  -- Выбор предыдущего ответственного
  select
    responsible_id
  into
    oldResponsibleId
  ...
  -- Установка статуса "Назначен"
  if oldResponsibleId is null then
    featureProcessId :=
      createFeatureProcessInternal(
        featureId         => featureId
      , featureStatusCode	=> Assigned_FeatureStatusCode
      , operatorId	      => operatorId
      );
  end if;
exception when others then
...
from
  ...
  select
    ...
    , f.work_sequence             as work_sequence
    , f.feature_name              as feature_name
    , f.feature_name_en           as feature_name_en
    , f.description               as description
    , p.feature_status_code       as feature_status_code
    , s.feature_status_name       as feature_status_name
    , s.feature_status_name_en    as feature_status_name_en
    , f.responsible_id            as responsible_id
    , r.operator_name             as responsible_name
    , r.operator_name_en          as responsible_name_en
    , f.date_ins                  as date_ins
    , f.operator_id               as operator_id
    , op.operator_name            as operator_name
    , op.operator_name_en         as operator_name_en
  from
    jrs_feature f
  left join
...
begin
  ...
  dsql.addCondition(
    'f.work_sequence >= :workSequenceFrom'
    , workSequenceFrom is null
    , 'workSequenceFrom'
  );
  dsql.addCondition(
    'f.work_sequence <= :workSequenceTo'
    , workSequenceTo is null
    , 'workSequenceTo'
  );
  ...
  dsql.addCondition(
    'f.operator_id = :authorId'
    , authorId is null
    , 'authorId'
  );
  dsql.addCondition(
    ''';''||:featureStatusCodeList||'';'' like ''%;''||s.feature_status_code||'';%'''
    , featureStatusCodeList is null
...
exception when others then
  raise_application_error(
    ...
    , logger.errorStack(
        ...
        || ', featureName="' || featureName || '"'
        || ', featureNameEn="' || featureNameEn || '"'
        || ', workSequenceFrom=' || to_char( workSequenceFrom)
        || ', workSequenceTo=' || to_char( workSequenceTo)
        || ', dateInsFrom=' || to_char( dateInsFrom, 'dd.mm.yyyy')
        || ', dateInsTo=' || to_char( dateInsTo, 'dd.mm.yyyy')
        || ', authorId=' || to_char( authorId)
        || ', responsibleId=' || to_char( responsibleId)
